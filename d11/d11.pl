...
%! inc_list(+Points, +Matrix, -NewMatrix)
...
%! tozero(+Matrix, +Point, -NewMatrix)
tozero(Matrix, (X, Y), NewMatrix) :-
    nth0(Y, Matrix, Line),
    replace(X, 0, Line, NewLine),
    replace(Y, NewLine, Matrix, NewMatrix).

%! tozero_list(+Points, +Matrix, -NewMatrix)
...
%! flash_list(+Points, +Matrix, -NewMatrix)
...
%! steps(+Steps, +Matrix, ++0, -TotalFlashes)
